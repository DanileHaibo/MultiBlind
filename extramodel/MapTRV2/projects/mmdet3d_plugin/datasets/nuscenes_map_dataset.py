    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                # Needed by the attack renderer to project global flare
                # locations into each camera image.
                global2ego = np.eye(4).astype(np.float32)
                global2ego[:3, :3] = Quaternion(
                    input_dict['ego2global_rotation']
                ).rotation_matrix.T
                global2ego[:3, 3] = (
                    -global2ego[:3, :3] @ input_dict['ego2global_translation']
                )
